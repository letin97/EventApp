...
import com.uit.letrongtin.eventapp.adapter.NewsAdapter;
import com.uit.letrongtin.eventapp.database.DataSource.TypeNewsRepository;
            ...
            private void isScrollCompleted() {
                if (totalItem - currentFirstVisibleItem == currentVisibleItemCount
                    ...
                    news.orderByChild("pubDate").startAt(oldestPostId).limitToFirst(15).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            for (DataSnapshot child : dataSnapshot.getChildren()) {
                                if (oldestPostId.equals(child.child("pubDate").getValue(String.class))) continue;
                                oldestPostId = child.child("pubDate").getValue(String.class);
